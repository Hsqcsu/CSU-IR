...
def extract_chonf_data(splits=['train', 'val', 'test']):
    # ================= 1. Path Settings =================
    ...
    for split in splits:
        ...
        out_sample_id_file = os.path.join(output_dir, f"{split}_source_chonf.txt")
        out_smiles_file = os.path.join(output_dir, f"{split}_description_chonf.txt")
        ...
        print(f"Loaded {total_samples} samples. Starting filtering and processing according to rules...")
        ...
        print(f"Saving processed {split} files...")
